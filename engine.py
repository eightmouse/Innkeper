...
def fetch_character(region: str, realm: str, name: str, token: str) -> dict | None:
    """Fetch full character profile — returns level, class, active_spec, etc."""
...
def fetch_character_media(region: str, realm: str, name: str, token: str) -> str | None:
    """Return best portrait URL — priority: main > main-raw > inset > avatar."""
    slug = realm.lower().replace(" ", "-").replace("'", "").replace(".", "")
    r = requests.get(
...
def _build_character(data: dict, region: str, realm: str, name: str, token: str) -> "Character":
    ...
    return Character(
        name, level, realm, region, portrait,
        class_id   = cls.get("id"),
...
class Character:
    def __init__(self, name, level, realm, region="eu",
                 portrait_url=None, class_id=None, class_name="", spec_name=""):
        self.name         = name
        self.level        = level
        self.realm        = realm
        self.region       = region
